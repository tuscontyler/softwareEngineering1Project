...
import Level1Package.Level1Window4;
import Level2Package.L2_W6;
import characterPackage.*;

public class SilverDragonFight extends BasicGameState {
	int oneTime;
	
	MouseListener hasPlayerClicked;
	
	Enemy foe = new SilverDragon();
	...
	public void init(GameContainer window, StateBasedGame sbg) throws SlickException {
		...
		bg = new Image("images/silverbg.png");
		
		textBox = new Rectangle(80, 370, 640, 200);

		bgm = new Music("music/BattleSong1.wav");
	...
	public void render(GameContainer window, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		g.drawString("Health: " + foe.getCurrentEnemyHealth() + " / " + foe.getMaxEnemyHealth(), 300, 200);
		...
		g.drawString("Silver Dragon defends its hoard!", 250, 400);
		...
		if (enemyDefeated) {
			...
			battleStarted = false;
			bgm.stop();
			L2_W6.setDragon(true);
		}
		
		else if (showingCantRun) {
			showCantRun(g);
		}
			...
			*/
			...
			if((posX > 540 && posX < 675) && (posY > 75 && posY < 105)) { //clickable area for BACK TO LEVEL
				if(Mouse.isButtonDown(0) && !showingDescription ) {
					sbg.enterState(8);	//return to level
	...
	public void showDescription(Graphics g) {
		g.drawString("The second strongest dragon of them all.", 250, 450);
